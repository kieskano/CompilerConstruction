...
public class GenericLLParser implements Parser {
	...
	private SymbolFactory fact = new SymbolFactory(Aba.class);
	...
	private AST parse(Symbol symb) throws ParseException {
        if (symb instanceof NonTerm) {
            ...
            Token next = next();
            if (next.getType() != ((Term) symb).getTokenType()) {
                throw new ParseException(String.format(
                        "Line %d:%d - expected token '%s' but found '%s'",
                        next.getLine(), next.getCharPositionInLine(),
                        fact.get(((Term) symb).getTokenType()), fact.get(next.getType())));
            }
	...
	private AST parse(Rule rule) throws ParseException {
	    ...
	    if (!rule.getRHS().get(0).equals(Term.EMPTY)) {
	...
	private Map<NonTerm, Map<Term, Rule>> calcLL1Table() {
        ...
        for (NonTerm nonTerm : g.getNonterminals()) {
            Map<Term, Rule> nonTermMap = new HashMap<>();
            for (Rule rule : g.getRules(nonTerm)) {
                for (Term fpTerm : calc.getFirstp().get(rule)) {
                    nonTermMap.put(fpTerm, rule);
                }
            }
            result.put(nonTerm, nonTermMap);
        }
